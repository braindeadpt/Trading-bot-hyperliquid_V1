...
# Feeds whose delivery evidence only exists while the bot itself runs — the
# bot PRODUCES them and no external source can regenerate the history after
# downtime. They stay contracted for the RUNTIME silence monitor (a recorder
# that stops persisting while the bot runs must still degrade), but they must
# never gate BOOT: after any downtime the evidence is stale by definition,
# and blocking startup on it deadlocks every restart that takes longer than
# the threshold (2026-08-14 audit).
#
# Verified same-situation candidates are NOT here: ``taker_split`` evidence
# (candles_1m buy/sell volume) is regenerated by candle backfill, which runs
# BEFORE the preflight at boot; ``microstructure`` persists no preflight
