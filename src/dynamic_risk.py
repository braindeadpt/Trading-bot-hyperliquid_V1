...
# =============================================================================
# FUNÇÃO DE TESTE / DEMO
# =============================================================================

def _demo():
    """Demonstra o comportamento do Dynamic SL/TP"""
    import random
    
    print("="*60)
    print("DEMONSTRAÇÃO: Dynamic SL/TP")
    print("="*60)
    
    # Simular candles com volatilidade de ~$1000 (1% de $100k)
    base_price = 100000
    candles = []
    price = base_price
    
    for i in range(30):
        change = random.uniform(-1500, 1500)
        open_p = price
        close_p = price + change
        high_p = max(open_p, close_p) + random.uniform(0, 500)
        low_p = min(open_p, close_p) - random.uniform(0, 500)
    ...
    print(f"ATR calculado: ${atr:,.2f} ({atr_pct*100:.2f}%)")
    print()
    
    # Simular uma posição LONG
    config = {'risk': {'initial_capital': 10000, 'max_position_size_usd': 100, 'max_leverage': 3, 'max_daily_trades': 5}}
    risk = DynamicRiskManager(config)
    risk.update_candles(candles)
    
    entry_price = 100000
    pos = risk.enter_position('BTC', 'long', entry_price, 100, 3)
    print()
    
    # Simular preço a subir
    test_prices = [
        99000,   # -1% — ainda seguro
        98000,   # -2% — ainda seguro (SL @ 97000)
        101000,  # +1% — BREAKEVEN! SL move para 100000
        102000,  # +2% — trailing começa
        103000,  # +3% — trailing folga 0.5xATR
        105000,  # +5% — trailing folga 1.0xATR
        104000,  # pullback — verificar se saiu
        108000,  # +8% — trailing folga 1.5xATR
        107000,  # pullback — verificar
        110000,  # +10% — TAKE PROFIT!
    ]
    
    for p in test_prices:
        print(f"\nPreço: ${p:,.0f} ({(p/entry_price-1)*100:+.1f}%)")
        result = risk.check_exit(p)
        if result:
            print(f"  >>> SAIU: {result['reason']} @ ${result['exit_price']:,.0f} | PnL: {result['pnl_pct']*100:+.1f}%")
            break
        else:
            status = pos.get_status()
            print(f"  SL: ${status['current_sl']:,.0f} | Max: ${status['max_price']:,.0f} | "
                  f"Gain: {status['highest_gain_pct']*100:.1f}% | Trail: {status['trailing_active']}")
