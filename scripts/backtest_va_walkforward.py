"""Walk-forward optimisation sweep for VA Rejection strategy (v3.1.34)."""
...
VA_SETS: List[ParamSet] = [
    ParamSet("baseline", {}),
    # --- VP window ---
    ParamSet("vp_48h", {"vp_lookback": 192}),  # 48h of 15m
    ParamSet("vp_12h", {"vp_lookback": 48}),
    # --- VA width ---
    ParamSet("va_60", {"va_pct": 0.60}),
    ParamSet("va_80", {"va_pct": 0.80}),
    # --- Balanced skip ---
    ParamSet("no_skip_balanced", {"skip_balanced": False}),
    ParamSet("strict_balanced", {"balanced_threshold": 0.50}),  # skip more
    # --- Volume / wick ---
    ParamSet("vol18_wick60", {"volume_surge": 1.8, "wick_rejection_min": 0.6}),
    ParamSet("vol13_wick50", {"volume_surge": 1.3, "wick_rejection_min": 0.5}),
    # --- ADX ---
    ParamSet("adx30", {"max_adx": 30.0}),
    ParamSet("adx50", {"max_adx": 50.0}),
    # --- TP R cap ---
    ParamSet("tp_2r", {"take_profit_r_multiple": 2.0}),
    ParamSet("tp_4r", {"take_profit_r_multiple": 4.0}),
    # --- Trailing ---
    ...
    ParamSet("aggressive", {
        ...
        "skip_balanced": False, "take_profit_r_multiple": 4.0,
    }),
]
...
def main() -> None:
    ...
    va_base = dict(cfg.get("strategy.va_rejection", {}) or {})

    all_rows: List[Dict[str, Any]] = []
    all_mc: Dict[str, Dict[str, Any]] = {}

    total = len(VA_SETS) * len(WINDOWS)
    done = 0
    t0 = time.time()
    print(f"VA walk-forward sweep: {total} runs ({len(VA_SETS)} param sets x 3 windows)\n")
    ...
    for ps in VA_SETS:
        ...
        for w_label, w_start, w_end in WINDOWS:
            ...
            print(f"  [{done}/{total}] VA {ps.name:32s} {w_label:18s} ...", end=" ", flush=True)
            t1 = time.time()
            res, trades = run_one(
                cfg, db, va_base, ps.overrides,
            ...
            if "error" in res:
                ...
                all_rows.append({
                    "strategy": "VA", "param_set": ps.name, "window": w_label,
            ...
            all_rows.append(row)
            all_mc.setdefault(f"VA|{ps.name}", {})[w_label] = mc_d
            print(
    ...
    csv_path = out_dir / f"va_walkforward_{ts}.csv"
    json_path = out_dir / f"va_walkforward_{ts}.json"
    cols = [
    ...
    print("\n=== VA ROBUST RANKING (walk-forward + Monte Carlo) ===")
    scored = []
    for ps in VA_SETS:
        key = f"VA|{ps.name}"
