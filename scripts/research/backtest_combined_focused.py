"""Combined focused sweep for v3.1.33-v3.1.37 strategies.

Runs a curated subset of param sets for SFP, VA, VB-retest, and ChecklistMeta
across the same 3 walk-forward windows, with Monte Carlo bootstrap.

Reduced from the full sweeps (which had 17+18+9+14 = 58 param sets = 174 runs)
to ~4 key sets per strategy = 16 sets = 48 runs (~60-90 min total).

Usage:
  $env:BOT_DATABASE_PATH = "data/live/bot_backtest.db"
  python scripts/research/backtest_combined_focused.py
"""
...
# Curated param sets — top candidates per strategy
SFP_SETS = [
    ParamSet("baseline", {}),
    ParamSet("vol18_wick50", {"volume_surge": 1.8, "wick_rejection_min": 0.5}),
    ParamSet("strict_sweep", {"volume_surge": 1.8, "wick_rejection_min": 0.6, "max_adx": 40.0}),
    ParamSet("strict_plus_trailing", {
...
VA_SETS = [
    ParamSet("baseline", {}),
    ParamSet("vp_48h", {"vp_lookback": 192}),
...
CL_SETS = [
    ...
    ParamSet("conservative", {
        "score_threshold": 3.5, "w_sfp": 2.0, "w_trend_structure": 1.5,
        "use_trailing_stop": True, "trailing_method": "ema9",
    }),
]
...
def main() -> None:
    ...
    bases = {
        "SFP": (SFPReversion, dict(cfg.get("strategy.sfp_reversion", {}) or {}), SFP_SETS),
        "VA": (VARejection, dict(cfg.get("strategy.va_rejection", {}) or {}), VA_SETS),
        "VBretest": (VolatilityBreakout, dict(cfg.get("strategy.volatility_breakout", {}) or {}), VB_RETEST_SETS),
    ...
    t0 = time.time()
    print(f"Combined focused sweep: {total} runs\n")
    ...
    for strat_name, (strat_cls, base, param_sets) in bases.items():
        ...
        for ps in param_sets:
            ...
            for w_label, w_start, w_end in WINDOWS:
                ...
                print(
                    f"  [{done}/{total}] {strat_name:10s} {ps.name:28s} {w_label:18s} ...",
                    end=" ", flush=True,
                )
                t1 = time.time()
                res, trades = run_one(
    ...
    csv_path = out_dir / f"combined_focused_{ts}.csv"
    json_path = out_dir / f"combined_focused_{ts}.json"
    cols = [
    ...
    print("\n=== COMBINED ROBUST RANKING ===")
    for strat_name, _, _, param_sets in [
        ("SFP", None, None, SFP_SETS),
        ("VA", None, None, VA_SETS),
        ("VBretest", None, None, VB_RETEST_SETS),
