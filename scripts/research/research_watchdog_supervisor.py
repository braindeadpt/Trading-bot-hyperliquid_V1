...
``nohup`` processes:
  ...
  * **Feed cadence** (``scripts/research/feed_cadence_diagnostic.py``) compares each
...
(no duplication): ``scripts/research/top_trader_bias_recheck.py`` and
``scripts/research/liquidation_flush_recheck.py`` remain the metric + report home.

Usage:
  python scripts/research/research_watchdog_supervisor.py            # daemon (every 6h)
  python scripts/research/research_watchdog_supervisor.py --once     # single check, exit
  python scripts/research/research_watchdog_supervisor.py --force    # run probes now
"""
...
from scripts.research.iv_gate_shadow_recheck import (  # noqa: E402
    IV_THRESHOLD,
