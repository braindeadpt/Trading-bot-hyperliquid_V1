...
This watchdog makes that re-run automatic and idempotent:
  ...
  * When span >= 30 days and no re-run has been recorded yet, it launches
    `scripts/research/liquidation_flush_shadow.py` (the full v2 simulation sweep),
...
SIM_SCRIPT = ROOT / "scripts" / "research" / "liquidation_flush_shadow.py"
...
def run_simulation() -> Optional[Path]:
    ...
    log("launching scripts/research/liquidation_flush_shadow.py (full sweep)")
    t0 = time.time()
    proc = subprocess.run(
...
def write_report(cell: Dict[str, Any], span_days: float, n_events: int,
    ...
    rows = [
        ...
        f"`scripts/research/liquidation_flush_recheck.py`._",
        "",
