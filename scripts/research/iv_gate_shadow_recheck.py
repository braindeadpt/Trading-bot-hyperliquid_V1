...
This watchdog makes the shadow→enforcement decision automatic and idempotent:
  ...
  * Every CHECK_HOURS (default 6h) it measures how many **closed** executed
    ...
    ``scripts/research/iv_gate_shadow_vs_pnl.py`` — the single source of truth).
...
Usage:
  python scripts/research/iv_gate_shadow_recheck.py            # daemon (every 6h)
  python scripts/research/iv_gate_shadow_recheck.py --once     # single check, exit
  python scripts/research/iv_gate_shadow_recheck.py --force    # run the comparison now
"""
...
from scripts.research.iv_gate_shadow_vs_pnl import (  # noqa: E402
    BACKTEST_EVIDENCE,
    MIN_N_GATE,
    build_report,
    resolve_db_paths,
...
def write_report(report: Dict[str, Any], v: Dict[str, Any],
    ...
    rows = [
        ...
        "`scripts/research/iv_gate_shadow_recheck.py`._",
        "",
        ...
        "",
        "## Slices (PnL realizado, via o join de `scripts/research/iv_gate_shadow_vs_pnl.py`)",
        "",
