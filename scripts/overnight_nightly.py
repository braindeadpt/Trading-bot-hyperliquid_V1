...
Usage::
    ...
    python scripts/overnight_nightly.py                 # queue-driven run
    python scripts/overnight_nightly.py --dry-run       # what would run, no backtests
    python scripts/overnight_nightly.py --family vwap_thresholds   # force a family
    python scripts/overnight_nightly.py --heartbeat     # cron keepalive only
...
def count_windows(window: Dict[str, Any]) -> int:
    ...
    Uses the same splitter as the sweep itself (scripts.regime_router_a_b_test)
