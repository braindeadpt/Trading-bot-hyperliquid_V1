"""Walk-forward sweep for exit optimizations (v3.1.39).

Goal: "let winners run maximally, cut losers early."

Three independent exit-tuning experiments across 3 walk-forward windows
+ 1000-iteration Monte Carlo bootstrap:

  1. ChecklistMeta: trailing stop + SL-to-BE + wider TP cap
     - baseline (current live: TP 3.0 ATR, no trail, no BE)
     - trail_ema9 (let winners run past 3R cap)
     - sl_to_be_1r (cut losers early once 1R green)
     - trail_ema9 + sl_to_be (both)
     - tp_6atr (wider TP cap)
     - all_combined

  2. VolatilityBreakout: SL-to-BE (trailing already ON from v3.1.28)
     - baseline_live (current: trailing_ema9, vol1.5)
     - sl_to_be_1r
     - sl_to_be_0.5r (earlier BE trigger)

  3. VWAPDeviation: configurable exit Z threshold
     - baseline (exit_z 0.3 = current)
     - exit_z_0.1 (let winners run to VWAP cross)
     - exit_z_0.2
     - exit_z_0.5 (exit earlier)

Robust criterion (same as v3.1.28/38):
  - n_trades >= 8 in ALL 3 windows
  - pf_p05 > 0.9 in ALL 3 windows
  - sharpe_p05 > -0.5 in ALL 3 windows
  - median across windows of pf_median >= 1.25

Outputs:
  data/backtests/exit_optimisation_<ts>.csv
  data/backtests/exit_optimisation_<ts>.json
  Console: robust ranking per strategy
"""
...
# --- VolatilityBreakout exit optimisations ---
VB_SETS: List[ParamSet] = [
    ParamSet("baseline_live", {}),
    ParamSet("sl_to_be_1r", {
        "use_sl_to_be_after_1r": True, "sl_to_be_r_trigger": 1.0,
    }),
    ParamSet("sl_to_be_0.5r", {
        "use_sl_to_be_after_1r": True, "sl_to_be_r_trigger": 0.5,
    }),
    ParamSet("sl_to_be_1r_plus_tp_6atr", {
        "use_sl_to_be_after_1r": True, "sl_to_be_r_trigger": 1.0,
        "take_profit_atr_multiplier": 6.0,
    }),
]

# --- VWAPDeviation exit Z threshold ---
VWAP_SETS: List[ParamSet] = [
    ParamSet("exit_z_0.3", {"exit_z_threshold": 0.3}),  # current default
    ParamSet("exit_z_0.1", {"exit_z_threshold": 0.1}),
    ParamSet("exit_z_0.2", {"exit_z_threshold": 0.2}),
    ParamSet("exit_z_0.5", {"exit_z_threshold": 0.5}),
    ParamSet("exit_z_0.0", {"exit_z_threshold": 0.0}),  # only exit at Z=0 (full VWAP cross)
]
...
def run_one(
    cfg: Any, db: Database, strategy_cls: type, base_section: Dict[str, Any],
    overrides: Dict[str, Any], window: str, start_ms: int, end_ms: int,
    ...
    # Compute avg winner / avg loser in USD
    win_pnls = [float(t.get("pnl_usd", 0)) for t in trades if float(t.get("pnl_usd", 0)) > 0]
    loss_pnls = [float(t.get("pnl_usd", 0)) for t in trades if float(t.get("pnl_usd", 0)) < 0]
    ...
    res = {
        ...
        # R:R realized = avg_win / |avg_loss|
        "rr_realized": round(abs(avg_win / avg_loss), 3) if avg_loss != 0 else 0.0,
    }
    return res, trades
...
def main() -> None:
    ...
    bases = {
        "CL": (ChecklistMeta, dict(cfg.get("strategy.checklist_meta", {}) or {}), CL_SETS),
        "VB": (VolatilityBreakout, dict(cfg.get("strategy.volatility_breakout", {}) or {}), VB_SETS),
        "VWAP": (VWAPDeviation, dict(cfg.get("strategy.vwap_deviation", {}) or {}), VWAP_SETS),
    }

    all_rows: List[Dict[str, Any]] = []
    all_mc: Dict[str, Dict[str, Any]] = {}

    total = sum(len(ps) for _, _, ps in bases.values()) * len(WINDOWS)
    done = 0
    t0 = time.time()
    print(f"Exit optimisation sweep: {total} runs\n")

    for strat_name, (strat_cls, base, param_sets) in bases.items():
        print(f"\n=== {strat_name} ===")
        for ps in param_sets:
            ...
            for w_label, w_start, w_end in WINDOWS:
                ...
                print(
                    f"  [{done}/{total}] {strat_name:5s} {ps.name:32s} {w_label:18s} ...",
                    end=" ", flush=True,
                )
                t1 = time.time()
                res, trades = run_one(
                ...
                print(
                    f"n={res['n_trades']:3d} PF={res['profit_factor']:>6} "
                    f"R:R={res['rr_realized']:>4.2f} WR={res['win_rate']:>5.1f}% "
                    f"mcPF_p05={mc.pf_p05:>5.2f} ProbP={mc.prob_profitable*100:>3.0f}% "
                    f"({row['elapsed_s']}s)"
                )
    ...
    csv_path = out_dir / f"exit_optimisation_{ts}.csv"
    json_path = out_dir / f"exit_optimisation_{ts}.json"
    cols = [
        "strategy", "param_set", "window", "elapsed_s",
        "n_trades", "wins", "losses", "win_rate",
        "avg_win_usd", "avg_loss_usd", "rr_realized",
    ...
    print("\n=== EXIT OPTIMISATION ROBUST RANKING ===")
    for strat_name, _, _, param_sets in [
        ("CL", None, None, CL_SETS),
        ("VB", None, None, VB_SETS),
        ("VWAP", None, None, VWAP_SETS),
    ]:
        print(f"\n--- {strat_name} ---")
        scored = []
        for ps in param_sets:
