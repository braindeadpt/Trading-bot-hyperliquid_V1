"""Walk-forward optimisation sweep for SFP Reversion strategy (v3.1.33).

Sweep matrix:
  - pivot_lookback (50, 100, 150)
  - volume_surge (1.3, 1.5, 1.8)
  - wick_rejection_min (0.5, 0.6)
  - max_adx (30, 40, 50)
  - magnet_retracement (0.5, 0.75, 1.0)
  - require_oi_confirm (true, false)
  - use_trailing_stop (false, ema9)

A param set is "robust" only if (same as v3.1.28 VB/VWAP sweep):
  - n_trades >= 8 in ALL 3 windows (statistical signal)
  - pf_p05 > 0.9 in ALL 3 windows (lower 5% bootstrap PF near breakeven)
  - sharpe_p05 > -0.5 in ALL 3 windows
  - median across windows of pf_median >= 1.25

Outputs:
  data/backtests/sfp_walkforward_<ts>.csv
  data/backtests/sfp_walkforward_<ts>.json
  Console: robust ranking
"""
...
SFP_SETS: List[ParamSet] = [
    ParamSet("baseline", {}),
    # --- Volume / wick strictness ---
    ParamSet("vol15_wick50", {"volume_surge": 1.5, "wick_rejection_min": 0.5}),
    ParamSet("vol18_wick50", {"volume_surge": 1.8, "wick_rejection_min": 0.5}),
    ParamSet("vol15_wick60", {"volume_surge": 1.5, "wick_rejection_min": 0.6}),
    ParamSet("vol18_wick60", {"volume_surge": 1.8, "wick_rejection_min": 0.6}),
    # --- ADX regime filter ---
    ParamSet("adx30", {"max_adx": 30.0}),
    ParamSet("adx50", {"max_adx": 50.0}),
    # --- Magnet target ---
    ParamSet("magnet_50", {"magnet_retracement": 0.5}),
    ParamSet("magnet_100", {"magnet_retracement": 1.0}),
    # --- Pivot lookback ---
    ParamSet("lookback_50", {"pivot_lookback": 50, "range_lookback": 50}),
    ParamSet("lookback_150", {"pivot_lookback": 150, "range_lookback": 150}),
    # --- OI confirm ---
    ParamSet("oi_confirm", {"require_oi_confirm": True}),
    # --- Trailing stop ---
    ParamSet("trailing_ema9", {
        "use_trailing_stop": True, "trailing_method": "ema9", "trailing_start_r": 1.0,
    }),
    # --- Combined "best" candidates ---
    ParamSet("strict_sweep", {
        "volume_surge": 1.8, "wick_rejection_min": 0.6, "max_adx": 40.0,
    }),
    ParamSet("strict_sweep_plus_trailing", {
        "volume_surge": 1.8, "wick_rejection_min": 0.6,
        "use_trailing_stop": True, "trailing_method": "ema9",
    }),
    ParamSet("conservative_sweep", {
        "volume_surge": 1.8, "wick_rejection_min": 0.6, "max_adx": 30.0,
        "magnet_retracement": 0.75,
    }),
    ParamSet("aggressive_sweep", {
        "volume_surge": 1.3, "wick_rejection_min": 0.5, "max_adx": 50.0,
        "magnet_retracement": 1.0,
    }),
]
...
def main() -> None:
    ...
    sfp_base = dict(cfg.get("strategy.sfp_reversion", {}) or {})

    all_rows: List[Dict[str, Any]] = []
    all_mc: Dict[str, Dict[str, Any]] = {}

    total = len(SFP_SETS) * len(WINDOWS)
    done = 0
    t0 = time.time()
    print(f"SFP walk-forward sweep: {total} runs ({len(SFP_SETS)} param sets x 3 windows)\n")
    ...
    for ps in SFP_SETS:
        ...
        for w_label, w_start, w_end in WINDOWS:
            ...
            print(
                f"  [{done}/{total}] SFP {ps.name:32s} {w_label:18s} ...",
                end=" ",
                flush=True,
            )
            t1 = time.time()
            res, trades = run_one(
                cfg, db, sfp_base, ps.overrides,
            ...
            if "error" in res:
                ...
                all_rows.append({
                    "strategy": "SFP", "param_set": ps.name, "window": w_label,
            ...
            all_rows.append(row)
            all_mc.setdefault(f"SFP|{ps.name}", {})[w_label] = mc_d
    ...
    csv_path = out_dir / f"sfp_walkforward_{ts}.csv"
    json_path = out_dir / f"sfp_walkforward_{ts}.json"
    ...
    # Robust ranking
    print("\n=== SFP ROBUST RANKING (walk-forward + Monte Carlo) ===")
    scored = []
    for ps in SFP_SETS:
        key = f"SFP|{ps.name}"
