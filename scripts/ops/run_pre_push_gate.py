...
keep the repo green:
  ...
  1. CI battery  - `scripts/ops/run_ci_tests.py` (unit + integration_offline;
...
# cheap. Must stay byte-identical to the snippet in scripts/ops/run_ci_tests.py.
_HASH_CHECK_SNIPPET = (
    "import json, sys; "
...
def main() -> int:
    ...
    parser.add_argument(
        "--preflight",
        action="store_true",
        help="also run scripts/ops/preflight_feed_check.py against the deployment "
    ...
    if args.preflight:
        preflight_cmd = [
            sys.executable,
            str(ROOT / "scripts" / "ops" / "preflight_feed_check.py"),
        ]
    ...
    ci_cmd = [
        sys.executable, str(ROOT / "scripts" / "ops" / "run_ci_tests.py"),
