...
Two modes (install via scripts/ops/install_git_hooks.py):
  ...
  --hook pre-push    Full gate — delegates to ``scripts/ops/run_pre_push_gate.py``
...
def _pre_push(root: Path) -> int:
    ...
    return subprocess.run(
        [sys.executable, str(root / "scripts" / "ops" / "run_pre_push_gate.py")],
...
def main() -> int:
    ...
    # against a temp git repo. scripts/ops/run_pre_push_gate.py and src/ are then
