#!/usr/bin/env python3
"""Re-register Fase 10 / Phase08 after making ``l2_recording`` hash-neutral.
...
Usage:
  python scripts/ops/reregister_phase10_l2_path_hash_neutral.py
...
REREG_REASON = (
    "research data-collection change only — l2_recording made hash-neutral "
    "(joins dvol_feed / feed_age_history in _sanitize_config_for_hash) and "
    "its path relocated back to the E: HDD, undoing the a26dee1 regression "
    "that wrote ~29 MB/day of research data to the system SSD and split the "
    "dataset. No strategy, risk, fee or execution_strategies change. OOS "
    "window PRESERVED — window_start_ms unchanged."
)
IN_SAMPLE_NOTE = (
    "Storage relocation only. VWAPDeviation remains the sole paper execution "
    "control (INCONCLUSIVE sample). No promotion; mainnet still blocked. "
    "Accumulated OOS evidence intentionally retained."
)
...
def _assert_hash_neutral(cfg: Config) -> None:
    ...
    for key, value in (("path", "X:/some/other/place"), ("interval_sec", 0.5)):
        raw = copy.deepcopy(cfg.raw)
        raw["market_data"]["l2_recording"][key] = value
        if compute_config_hash(Config(raw)) != base:
            raise SystemExit(
                f"Refusing re-register: l2_recording.{key} still changes the "
...
def main() -> int:
    ...
    l2_path = str(cfg.get("market_data.l2_recording.path", ""))
    ...
    print("Phase10 re-registered OK (l2_recording hash-neutral + path -> E:)")
    ...
    print(f"  l2_recording.path:    {l2_path}")
