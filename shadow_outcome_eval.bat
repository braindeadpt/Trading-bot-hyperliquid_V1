@echo off
chcp 65001 >nul
title Shadow Outcome Evaluation (hourly)
...
:: is still active exits 0 with no work. Safe to run hidden hourly.
::
:: Proposed registration (NOT executed by this file):
::   schtasks /Create /TN "Hyperliquid Shadow Outcome Eval" /SC HOURLY /MO 1 /ST 00:20 /F ^
