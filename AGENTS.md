...
| Web framework | `flask==3.1.3`, `flask-socketio==5.6.1`, `python-socketio==5.13.0` |
...
| **systemd owns liveness** | `Restart=always` replaces `run_with_recovery.py` and the Bot-Alive task. The six Windows scheduled tasks map to `.timer` units (`deploy/systemd/`); `MemoryMax=3G` on the bot. `CPUWeight` values are a **machine-level decision owned by the Meridian repo's `docs/VPS-SETUP.md` §6a** — bot is 1000 (top weight, the only real-time service); never tune a unit in isolation. |
| **Python 3.13 via uv** | Ubuntu 24.04's default 3.12 is too old. `install.sh` uses `uv python install 3.13` — every pinned dep (numpy/pandas/scipy included) ships a cp313 aarch64 wheel, so the install is all-binary with the same pins as Windows (runbook §1a). On 3.14 the scientific stack has no wheels — do not switch without re-auditing PyPI. |
